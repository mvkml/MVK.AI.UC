---
name: dev-angular
description: Use for frontend work in ai_hr — building/modifying Angular components in hr_ui/mvkhrweb and mvkhrapp, integrating with the FastAPI backend, and Graphify UI visualizations. Invoke for any UI feature, page, or component work.
...
## Responsibilities
- Develop Angular components for `mvkhrweb` and `mvkhrapp`
...
- Match the existing dark/purple theme already used in `mvkhrweb` (see `upload.scss` for the color variables: `$bg`, `$card`, `$purple`, `$blue`, `$green`, `$muted`, `$border`, `$gradient`).
